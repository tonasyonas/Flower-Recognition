...
def train_deep_model(epochs=50, batch_size=32, device="cpu"):
    print(f"Initializing VPT-Deep training on {device}...")

    set_seed()

    train_loader, val_loader, test_loader = get_dataloaders(batch_size=batch_size)

    model = ViTDeepForFlowerRecognition(num_classes=102, num_prompts=10).to(device)
    ...
    print("Starting VPT-Deep Training Loop...")
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), "results/best_vit_vpt_deep_model.pth")

    print("VPT-Deep Training Complete. Best Validation Accuracy:", best_val_acc)


if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    train_deep_model(epochs=50, batch_size=32, device=device)
