...
We apply test-time augmentation by averaging softmax predictions over five views (original, horizontal flip, $\pm 5^\circ$ rotation, $+10^\circ$ rotation). TTA improves every method by 0.4--0.9 points at no extra training cost (Table~\ref{tab:tta}); VPT-Deep + TTA reaches 93.27\% MPC, our highest test number. To address the single-seed limitation we re-train the three best methods with seeds 7 and 42 in addition to seed 69 (Table~\ref{tab:seeds}). The relative ranking VPT-Deep $>$ Frozen $>$ Baseline holds at every seed, and standard deviations stay below one point.
