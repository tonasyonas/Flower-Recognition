...
def train_one_epoch(model, dataloader, criterion, optimizer, device, use_mixup=True):
    ...
    for inputs, targets in progress_bar:
        ...
        if use_mixup:
            # Apply MixUp
            mixed_inputs, targets_a, targets_b, lam = mixup_data(inputs, targets, alpha=0.2, use_cuda=device.type=='cuda')
            ...
            correct += (lam * predicted.eq(targets_a.data).sum().float() + (1 - lam) * predicted.eq(targets_b.data).sum().float())
...
def train_model(epochs=50, batch_size=32, device='cpu'):
    ...
    for epoch in range(epochs):
        ...
        train_loss, train_acc = train_one_epoch(model, train_loader, criterion, optimizer, device, use_mixup=use_mixup)
...
if __name__ == '__main__':
    ...
    print("Training loop logic written. Script is set to skip execution to prevent CPU blocking.")
